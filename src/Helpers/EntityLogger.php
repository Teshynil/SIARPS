...
//            }
//        }
        foreach ($uow->getScheduledEntityInsertions() as $cUpdates) {
            if ($cUpdates instanceof \App\Entity\Version) {
                $project=$cUpdates->getDocument()->getProject();
                $project->setLastUpdate();
                $em->persist($project);
                $uow->computeChangeSet($em->getClassMetadata(get_class($project)), $project);
            }
        }
    }
